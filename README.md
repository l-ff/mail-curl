...
| `temp_mail_io` | 通过 temp-mail.io JSON 接口生成邮箱并读取消息列表/正文 | 设置 `TEMP_MAIL_IO_ENABLED=1` |
| `mail_tm` | 通过 mail.tm JSON/Bearer 接口创建邮箱并读取消息列表/正文 | 设置 `MAIL_TM_ENABLED=1` |
| `priyo_email` | 通过 priyo.email Web/Livewire 页面流程切换邮箱并解析 HTML 收件箱/邮件详情 | 设置 `PRIYO_EMAIL_ENABLED=1` |
...
TEMP_MAIL_IO_ENABLED=1
MAIL_TM_ENABLED=1
PRIYO_EMAIL_ENABLED=1
