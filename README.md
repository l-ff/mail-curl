...
- 支持通过 `all` 随机选择一个已启用 provider 创建邮箱
- 支持共享 API key 鉴权
- 使用 `dotenv` 从 `.env` 和 `.env.local` 加载配置
- 支持 Docker 部署
...
| Provider | 说明 | 启用方式 |
| --- | --- | --- |
| `chatgpt_mail` | 通过 HTTP API 接入 ChatGPT Mail | 设置 `CHATGPT_MAIL_API_KEY` 后启用 |
| `twenty_four_email` | 通过 24.email 生成邮箱、读取 WebSocket 收件箱并解析邮件 HTML | 默认启用，设置 `TWENTY_FOUR_EMAIL_ENABLED=0` 禁用 |
| `generator_email` | 通过 generator.email 生成邮箱并解析 HTML 收件箱/邮件详情 | 默认启用，设置 `GENERATOR_EMAIL_ENABLED=0` 禁用 |
| `temp_mail_io` | 通过 temp-mail.io JSON 接口生成邮箱并读取消息列表/正文 | 默认启用，设置 `TEMP_MAIL_IO_ENABLED=0` 禁用 |
| `mail_tm` | 通过 mail.tm JSON/Bearer 接口创建邮箱并读取消息列表/正文 | 默认启用，设置 `MAIL_TM_ENABLED=0` 禁用 |
| `priyo_email` | 通过 priyo.email Web/Livewire 页面流程切换邮箱并解析 HTML 收件箱/邮件详情 | 默认启用，设置 `PRIYO_EMAIL_ENABLED=0` 禁用 |
...
服务通过 `dotenv` 依次读取 `.env` 和 `.env.local`，但不会覆盖已经存在的系统环境变量。
...
| `CHATGPT_MAIL_ENABLED` | `1` | 设置为 `0` 后禁用 `chatgpt_mail` provider |
| `CHATGPT_MAIL_BASE_URL` | `https://mail.chatgpt.org.uk` | `chatgpt_mail` 上游地址 |
| `CHATGPT_MAIL_TIMEOUT_MS` | `10000` | `chatgpt_mail` 请求超时时间 |
| `TWENTY_FOUR_EMAIL_ENABLED` | `1` | 设置为 `0` 后禁用 `twenty_four_email` provider |
...
| `GENERATOR_EMAIL_ENABLED` | `1` | 设置为 `0` 后禁用 `generator_email` provider |
...
| `TEMP_MAIL_IO_ENABLED` | `1` | 设置为 `0` 后禁用 `temp_mail_io` provider |
...
| `MAIL_TM_ENABLED` | `1` | 设置为 `0` 后禁用 `mail_tm` provider |
...
| `PRIYO_EMAIL_ENABLED` | `1` | 设置为 `0` 后禁用 `priyo_email` provider |
...
# 已适配的 provider 默认启用；需要关闭时设置对应 ENABLED=0
# TWENTY_FOUR_EMAIL_ENABLED=0
# GENERATOR_EMAIL_ENABLED=0
# TEMP_MAIL_IO_ENABLED=0
# MAIL_TM_ENABLED=0
# PRIYO_EMAIL_ENABLED=0
...
随机选择一个已启用 provider 创建邮箱：

```bash
curl -X POST "http://localhost:3100/all/api/remail?key=your-secret-key"
```

`all` 返回的 `id` 会带真实 provider 前缀，后续可继续用于 `/all/api/inbox` 和 `/all/api/mail`。
...
`twenty_four_email` 可以替换为任意已启用 provider 的名称。使用 `all` 时，`mailbox_id` 和 `id` 需要传入 `/all/api/remail` 或 `/all/api/inbox` 返回的带 provider 前缀的值。
